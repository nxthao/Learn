...
#include<stdbool.h> 
/*
Print position with search node
*/
...
bool SearchNodeWithKey(Node *node, int key, int *pos)
{ 
	int i = 0;
	while (node->data != key)
	{
		...
		node = node->next;
		i++;// I unable to use *pos++, because I don't assign the memory for it
	}
	*pos = i;
	return true;
}
...
void main()
{
	...
	search = SearchNodeWithKey(head, key, &pos);
